...
function cleanTR(suiteTests, test) {
  console.log('reduce', suiteTests.length, test.title);
  if (/TR-([0-9]*)/.exec(test.title) && !test.pending) {
    ...
    var elapsed = 0;
    if (test.duration) {
      elapsed = Math.round(test.duration/1000);
    }
    ...
    if (test.duration) {
      o.elapsed = Math.round(test.duration/1000) + "s";
    }
    if (test.hasOwnProperty("err")) {
      o.message = test.err.message.toString();
    }
    ...
    function recursiveIteration(object, callback) {
      if (object.hasOwnProperty('suites')) {
        if (object['suites'].length > 0){
          ...
          callback(object, 'suites');
        }
      }
    }

    function test_cb(suite, property){
      suiteTests = suiteTests.concat(suite.tests);
    }
